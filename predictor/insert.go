...
func SaveInfo(info Info, source models.Source) {
	if info.Embed.IsNew() {
		InsertNewInfo(info, source)
	} else {
		updateExistingInfo(info, source)
	}
}

func InsertNewInfo(info Info, source models.Source) {
...
func updateExistingInfo(info Info, source models.Source) {
